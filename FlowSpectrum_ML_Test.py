...
from sklearn.decomposition import PCA,TruncatedSVD,FastICA,KernelPCA,FactorAnalysis,DictionaryLearning
...
if __name__=='__main__':
    ...
    testset = pd.read_csv('KDDtest__.csv', index_col=0)
    ...
    BaseModels=[
        ...
        ('FactorAnalysis',FactorAnalysis(n_components=1)),
        ('FastICA',FastICA(n_components=1)),
        ('LDA',LinearDiscriminantAnalysis(n_components=1)), 
    ]
    for ModelName,Encoder in BaseModels:
        print(ModelName)
        Encoder.fit(X=X_train_,y=y_train)
        FS=FlowSpectrum(
            X=X_train_,y=y_train,
            Labels=Labels,
            reconstructor=Encoder,
        )
        # with open(f'Result\{ModelName}_FS.pickle','wb') as f:
        #     pickle.dump(FS.FlowSpectrum,f)
        FS.get_FlowSpectrum(
            title=f'FlowSpectrum Based on {ModelName}',
            # save_path=f'FlowSpectrum/{ModelName}_FS.png'
        )
        FS.detection_test(
            X_test=X_, y_test=y,
